...
branch_edit() {
  ...
  if ! $found_master && ! $found_main; then
    echo "zsh: main/master branches not found for command: ${executed_command}"
    return 1
  fi
}
...
branch_fixer() {
  ...
  local executed_command=$(fc -ln -1)

  if [[ $exit_code != 0 ]]; then

    # Split the string based on the delimiter, ':'
    strarr=($(echo "$executed_command" | awk -v RS='[[:space:]]+' '{for (i=1; i<=NF; i++) print $i}'))
    printf "\n"
